...
I chose to use Map<Integer, Double> to represent a Polynomial, where the key is the degree and the value is the coefficient.

This representation is easy to work with because polynomials may have gaps in their
degrees, and using a map allows us to store only the non-zero terms. It makes kernel method easier to execute two method which are setCoefficient and getCoefficient. Computing the degree of the polynomial can be done by taking the maximum key in the map.

Convention:
key means the exponent of x, they should be non-negative and and all coefficients are values, they must be valid (non-null) real numbers. By the way, if key is same the polynomial can not appear multiple times. 

Correspondence:
This map represents the mathematical polynomial 
Σ (map.get(d) * x^d) for each degree d contained in the map.
...
- Designed kernel implementation for Polynomial component
